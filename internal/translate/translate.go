...
//
// At S04 the supported subset is `root`, `paragraph`, `heading{depth}`,
// `text{value}`, `emphasis`, and `strong`. Later slices fill in the rest of
// the enumerated v1 node set (lists, code, links, tables, footnotes, etc.).
//
// Output type: a Go value tree (pointers to `Node` structs), NOT goldmark's
// native AST. This is the structural guarantee acceptance criterion #4 from
// S03 pins: after translate, the rest of the pipeline never sees goldmark
// types.
package translate
...
// Options controls per-translate behavior. At S04 there are still no
// per-translate knobs; the struct is kept so later slices can add per-node
...
// policy").
//
// At S06 the recognized set is: Heading, Paragraph, Text, Emphasis (level 1 →
// emphasis, level 2 → strong), List, ListItem, TextBlock (goldmark's tight-list
// inline-container wrapper; flattened to its inline children), Blockquote,
// ThematicBreak, FencedCodeBlock, CodeBlock (indented), HTMLBlock, RawHTML
// (inline), CodeSpan, Link, Image. The synthetic `break` node for hard line
// breaks is emitted by `translateChildren` based on the preceding Text node's
// HardLineBreak() flag, not by a goldmark node of its own (goldmark stores the
// hard-break signal on the Text node, not as a sibling).
