# PRD: md2json — a small CLI that converts Markdown to JSON
...
`md2json` is a single-binary Go CLI that reads one Markdown document (from `stdin` or a positional `FILE`) and writes one JSON document to `stdout`. The output is a fixed envelope `{"frontmatter": <object>|null, "ast": <mdast root node>}`: YAML frontmatter (if present) is lifted out as a first-class top-level field, and the body is parsed and emitted as an `mdast`-shaped node tree drawn from a closed, documented v1 node set. The tool is a Unix filter first — compact JSON by default, stdout reserved for the JSON document, errors on stderr with a non-zero exit code, and one consistent stderr regex across every diagnostic. It installs via `go install …@latest` or as a prebuilt static binary from GitHub Releases, with no Node / Python / Haskell runtime to manage.

## User Stories

1. As a blog author with a directory of `.md` posts, I want to run `md2json post.md` and get a single-line JSON envelope on stdout, so that I can pipe it into `jq` to extract the title or tags from the frontmatter.
2. As a shell scripter, I want to run `md2json < post.md > post.json` (no positional argument), so that the tool behaves as a Unix filter and composes with `cat`, `xargs`, and process substitution.
3. As a CI author processing many files, I want `md2json -o out.json post.md` to write directly to a file, so that I do not need to manage shell redirection for every invocation.
...
18. As a programmer parsing diagnostics, I want every stderr line to match exactly one regex `^md2json: ([^:]+):(\d+):(\d+): (.+)$`, so that I can `grep`/`sed` paths and positions out without writing a multi-format parser.
...
20. As a programmer running the tool with no input source determined yet (usage error: unknown flag, missing required positional, or unreadable `FILE` before any bytes are read), I want the `<path>` token in the error line to be the literal program name `md2json` and the position to be the `:0:0:` "no position available" sentinel, so that the canonical stderr regex still matches and I can distinguish pre-input usage errors from document-scoped errors by inspecting `<path>`.
21. As a programmer whose document uses YAML frontmatter, I want a malformed frontmatter block (tab indent, unbalanced quotes, duplicate keys) to fail loudly with `md2json: <path>:<line>:<col>: invalid frontmatter: <yaml error>` and exit `1`, so that bad metadata never silently disappears into a `frontmatter: null` envelope.
...
24. As a programmer with a file containing invalid UTF-8 bytes, I want the tool to exit `1` with `md2json: <path>:<line>:<col>: invalid utf-8 byte at offset <N>` on stderr (and never silently substitute U+FFFD), so that a partially-corrupt input never produces a partially-corrupt AST that looks fine.
25. As a programmer running `md2json --no-position < empty.md` (zero-byte input), I want exactly `{"frontmatter":null,"ast":{"type":"root","children":[]}}` on stdout and exit `0`, so that the empty-document baseline is byte-for-byte stable.
26. As a programmer running `md2json < empty.md` (default, zero-byte input), I want the same envelope but with a zero-width `position` on `root` (`{"start":{"line":1,"column":1,"offset":0},"end":{"line":1,"column":1,"offset":0}}`) and exit `0`, so that the "every node carries `position` by default" rule holds without per-node special-casing.
...
31. As an installer, I want to run `go install github.com/<owner>/md2json@latest` and get a working binary on my `PATH`, so that I do not need to deal with Node, Python, or Haskell runtimes.
...
| ----------------------------------------------- | -------------------------------------------------------------------- | ---- |
| Pre-input usage error (unknown flag, missing positional, unreadable `FILE` before any bytes read) | `md2json: md2json:0:0: <usage message>` | `2`  |
| Document-scoped error (goldmark error with no line/column) | `md2json: <path>:0:0: <message>` | `1`  |
| Invalid UTF-8 byte                              | `md2json: <path>:<line>:<col>: invalid utf-8 byte at offset <N>`    | `1`  |
| Malformed YAML frontmatter                      | `md2json: <path>:<line>:<col>: invalid frontmatter: <yaml error>`   | `1`  |
| goldmark unrecoverable parse error              | `md2json: <path>:<line>:<col>: <message>`                           | `1`  |
| Success                                         | (nothing on stderr; JSON on stdout)                                  | `0`  |

When goldmark reports a line but no column, print `<path>:<line>:1:` (round unknown column up to 1, never 0). When the error is document-scoped with no position, use the `:0:0:` sentinel per CONTEXT.md's `Error format` entry. When the input is stdin (and `read` has been entered), `<path>` is the literal `-`. When the failure occurs before any input source is determined — usage errors raised by `cli` flag parsing or by failing to open a `FILE` for reading — `<path>` is the literal program name `md2json` (so the line reads `md2json: md2json:0:0: ...`). The literal `-` is reserved for "stdin was the chosen source"; the literal `md2json` is the sentinel for "no source was ever in play." The single regex `^md2json: ([^:]+):(\d+):(\d+): (.+)$` matches every stderr line; consumers distinguishing pre-input from document-scoped errors inspect `<path>` (`md2json` vs file path or `-`).
...
### What makes a good test for md2json
...
- **The v1 ship criterion is the headline acceptance test.** It decomposes into two concrete fixtures: (1) `md2json --no-position < empty.md` produces exactly `{"frontmatter":null,"ast":{"type":"root","children":[]}}` on stdout and exit `0`; (2) `md2json < empty.md` (default) produces the same envelope with a zero-width `position` field on `root` and exit `0`. Both must hold byte-for-byte.
...
- **One fixture per failure mode** in the error/exit-code table above: pre-input usage error (asserting the `<path>` token is literally `md2json` and the position is `:0:0:`), invalid UTF-8, malformed frontmatter, unrecoverable goldmark error, document-scoped goldmark error (asserting the `:0:0:` sentinel and a file-path `<path>`). Each asserts the exact stderr line matches the canonical regex and the exit code.
...
- Watch mode.
- Config file (no `.md2jsonrc`, no env vars in v1).
