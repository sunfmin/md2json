...
$ printf '# Title\n\nBody with **bold**.\n' | /tmp/md2json --pretty --no-position | head -8
{
  "frontmatter": null,
  "ast": {
    "type": "root",
    "children": [
      {
        "type": "heading",
        "depth": 1,
$ printf '    abc\n    def\n' | /tmp/md2json --pretty --no-position | grep -E '"(lang|meta)":'
        "lang": null,
        "meta": null,
$ printf '- plain\n' | /tmp/md2json --pretty --no-position | grep '"checked"'
            "checked": null,
$ printf '[no-title](https://example.com)\n' | /tmp/md2json --pretty --no-position | grep '"title"'
            "title": null,
$ printf 'foo\n' | /tmp/md2json --pretty --no-position > /tmp/pretty.json
$ printf 'foo\n' | /tmp/md2json --no-position > /tmp/compact.json
