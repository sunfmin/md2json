...
$ /tmp/md2json --no-position < /dev/null | xxd
...
$ /tmp/md2json --frontmatter-only < /dev/null
null
```
...
- Acceptance criteria status:
  - [x] criterion 1 — `md2json --no-position < empty.md` exact envelope, exit 0 (`TestEmptyStdinNoPositionEmitsEnvelope`, fixture `01-empty-stdin`)
  - [x] criterion 2 — `md2json < empty.md` exact envelope with `position` on root, exit 0 (`TestEmptyStdinDefaultEmitsEnvelopeWithPosition`, fixture `10-empty-stdin-default`)
