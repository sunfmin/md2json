...
- **Change**: introduce a private helper `writeDocScopedError(stderr io.Writer, pathToken string, err error)` that wraps the `fmt.Fprintf(stderr, "md2json: %s:0:0: %s\n", pathToken, err.Error())` template. Replace all four inline `fmt.Fprintf` call sites with the helper.
