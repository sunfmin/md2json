...
- Module path: `github.com/sunfmin/md2json` (matches the repository owner; per CONTEXT.md the canonical install path is `go install github.com/<owner>/md2json@latest`).
- Layout chosen:
...
- Red: `go test .` reported `no non-test Go files in /Users/sunfmin/Developments/md2json` — the `TestMain` build step failed because no `main.go` existed yet.
...
- `go vet ./...`: clean.
- Manual end-to-end verification: `go build -o /tmp/md2json . && /tmp/md2json < /dev/null | xxd` returns exactly the 56-byte envelope with no trailing newline, exit 0 — the v1 ship criterion's `--no-position` flavor (S01 hard-codes the envelope, so this holds unconditionally at this stage).

- Acceptance criteria status:
  - [x] criterion 1 — `md2json < /dev/null` emits exact envelope, exit 0 (`TestEmptyStdinEmitsHardcodedEnvelope`, fixture `01-empty-stdin`)
