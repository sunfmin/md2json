...
- [ ] Fixture: `md2json --no-position` on any non-empty fixture produces output identical to the default mode but with the `position` key absent from **every** node (root and all descendants); no other key ordering or field-presence behavior changes.
