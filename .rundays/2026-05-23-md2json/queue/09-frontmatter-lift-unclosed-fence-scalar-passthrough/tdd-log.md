...
- Baseline check (run before writing any new test):
  ...
  - Malformed YAML between closed fences: currently emits `md2json: -:0:0: yaml: line 3: ...` (the doc-scoped catch-all branch from S03's `cli.Run`). WRONG per CONTEXT.md "Invalid frontmatter (policy)" + PRD user story 21 (criterion #3).
...
- Wrote: fixture `testdata/fixtures/51-frontmatter-only-malformed-yaml/` with `input.md = "---\ntitle: \"unclosed\n---\n"`, args `--frontmatter-only`, expected stdout empty, expected stderr identical to fixture `46-malformed-yaml-hard-error` (`md2json: -:3:1: invalid frontmatter: found unexpected end of stream\n`), exit `1`.
...
```
$ go build -o /tmp/md2json .
$ printf -- "---\ntitle: x\n---\n\nbody\n" | /tmp/md2json --no-position
...
$ printf -- "---\ntitle: \"unclosed\n---\n" | /tmp/md2json
md2json: -:3:1: invalid frontmatter: found unexpected end of stream
$ printf -- '---\n"hello"\n---\n' | /tmp/md2json --frontmatter-only
"hello"
$ printf -- "---\n42\n---\n" | /tmp/md2json --frontmatter-only
42
$ printf -- "---\nnull\n---\n" | /tmp/md2json --frontmatter-only
...
md2json: -:3:1: invalid frontmatter: found unexpected end of stream
$ printf -- "---\ntitle: x\ntitle: y\n---\n" | /tmp/md2json --no-position
md2json: -:3:1: invalid frontmatter: mapping key "title" already defined at line 1
```
...
- Acceptance criteria status:
  ...
  - [x] criterion 3 — malformed YAML between closed fences writes canonical `md2json: <path>:<line>:<col>: invalid frontmatter: <yaml error>` stderr, exit 1, empty stdout (`TestFixtures/46-malformed-yaml-hard-error`, `TestParseMalformedYAMLClosedFenceReturnsTypedError`).
