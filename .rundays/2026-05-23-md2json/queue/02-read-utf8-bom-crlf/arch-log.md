...
CONTEXT.md "Input handling" specifies the exact stderr message for invalid UTF-8: `md2json: <path>:<line>:<col>: invalid utf-8 byte at offset <N>`. Verified:
- `ReadError.Msg` is constructed as `"invalid utf-8 byte at offset %d"` — matches verbatim.
- `(*ReadError).Error()` returns `"<path>:<line>:<col>: <msg>"` — matches the per-line shape, leaving the `md2json: ` tool-name prefix to the cli layer (where it belongs — the read module is deliberately tool-agnostic per its package docstring).
- `ReadError.{Path, Line, Col, Offset, Msg}` field names tokenize the regex `^md2json: ([^:]+):(\d+):(\d+): (.+)$` directly.
