...
- Green: created `internal/read/read.go` with `Read`/`*ReadError`, wired `cli.Run` to call it (path token `-` for stdin, file path otherwise), and routed the typed error through `fmt.Fprintf(stderr, "md2json: %s\n", re.Error())` + `return 1`. The S01 envelope-on-success path is preserved on the no-error branch.
...
- Wrote: `TestMidDocumentInvalidUTF8StdinHardErrors` — input `"hi\nworld\xC3\x28"`, expects `md2json: -:2:6: invalid utf-8 byte at offset 8\n` and exit 1.
...
- `go vet ./...`: clean.
- Manual end-to-end verification: `printf '\xFF' | /tmp/md2json` writes `md2json: -:1:1: invalid utf-8 byte at offset 0\n` to stderr, nothing to stdout, exit 1. Behavior matches the canonical contract.

- Acceptance criteria status:
  - [x] criterion 1 — leading-0xFF fixture: stderr `md2json: -:1:1: invalid utf-8 byte at offset 0`, exit 1, empty stdout (`TestFixtures/02-invalid-utf8-leading`, in-process `TestLeadingInvalidUTF8StdinHardErrors`)
  - [x] criterion 2 — mid-doc 0xC3 0x28 fixture: stderr `md2json: -:2:6: invalid utf-8 byte at offset 8`, exit 1 (`TestFixtures/03-invalid-utf8-mid-document`, in-process `TestMidDocumentInvalidUTF8StdinHardErrors`)
