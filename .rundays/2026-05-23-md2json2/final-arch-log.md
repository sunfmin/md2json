...
- `go test ./...` (count=1, no cache): all 6 packages OK
  ...
  - per-package: `github.com/sunfmin/md2json2`, `internal/cli`, `internal/emit`, `internal/parse`, `internal/read`, `internal/translate` — all `ok`
...
- **Sp-1. `cli.preInputPathToken` constant** named for one of one callers. The literal `"md2json2"` also appears in the canonical-error regex test fixtures, the integration test (`m[1] != "md2json2"`), and the release-workflow YAML (asset name `md2json2-...`). A single shared constant across boundaries would over-couple — keep one per language site.
