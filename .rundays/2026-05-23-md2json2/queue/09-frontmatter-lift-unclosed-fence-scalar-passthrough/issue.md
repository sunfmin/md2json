...
- [ ] Fixture: a document with closed `---` fences containing malformed YAML (e.g. unbalanced quotes) writes `md2json2: <path>:<line>:<col>: invalid frontmatter: <yaml error>` to stderr, exits `1`, with nothing on stdout.
- [ ] Fixture: `md2json2 --frontmatter-only` on `--- "hello" ---` writes exactly `"hello"` (plus newline) to stdout, exit `0`.
- [ ] Fixture: `md2json2 --frontmatter-only` on `--- 42 ---` writes `42`; on `--- null ---` writes `null`; on a document with no frontmatter writes `null`.
- [ ] Fixture: `md2json2 --frontmatter-only` on the malformed-YAML document emits the same `invalid frontmatter` stderr line and exit `1` (the flag does not change the upstream error contract).
