...
> unbalanced quotes, duplicate keys, etc.), write `md2json2:
...
> This is the single rendering point for the `md2json2: <path>:0:0:
...
"Error format" template `^md2json2: ([^:]+):(\d+):(\d+): (.+)$` was
...
- Change:
  ...
  - In `Run`: the `*read.ReadError` branch now calls
    ...
    instead of `fmt.Fprintf(stderr, "md2json2: %s\n", re.Error())`.
...
- Most consequential change: cli now renders the canonical CONTEXT.md
  "Error format" template (`^md2json2: ([^:]+):(\d+):(\d+): (.+)$`)
