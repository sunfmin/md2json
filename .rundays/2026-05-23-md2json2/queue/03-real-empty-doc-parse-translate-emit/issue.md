...
- [ ] `md2json2 --no-position < empty.md` (zero-byte input) writes exactly `{"frontmatter":null,"ast":{"type":"root","children":[]}}` to stdout (no trailing newline) and exits `0`. This is the v1 ship criterion's first half.
- [ ] `md2json2 < empty.md` (default, no `--no-position`) writes `{"frontmatter":null,"ast":{"type":"root","children":[],"position":{"start":{"line":1,"column":1,"offset":0},"end":{"line":1,"column":1,"offset":0}}}}` to stdout (no trailing newline) and exits `0`. This is the v1 ship criterion's second half.
- [ ] The goldmark parser is constructed with GFM, frontmatter, and footnote extensions registered; running `md2json2 --frontmatter-only < empty.md` writes `null` to stdout and exits `0`.
