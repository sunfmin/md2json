...
`md2json` reads a single Markdown document and emits a JSON envelope whose nodes carry `position` (line / column / offset). Several upstream decisions about *how the raw bytes become a parseable, position-stable document* shape every downstream guarantee — the position values, the cross-platform stability of `line`/`column`/`offset`, the error behavior on bad bytes, and the OOM surface. These decisions need to be discoverable to a future maintainer (and to TDD-stage test authors) as a single record with rationale, not scattered across a glossary.
...
- **Positive (fail-fast correctness).** Bad bytes never silently degrade output. The contract "if md2json exits 0, the AST is faithful to the input" holds.
